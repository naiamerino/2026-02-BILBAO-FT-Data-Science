...
with tab3:
    ...
    if not st.session_state.rg_inicializado:
        ...
        # Que lo haga una única vez por ronda. No cada vez que pulsamos un botón
        clave_cache = f"partidos_ronda_{ronda_idx}"
        if clave_cache not in st.session_state:
            ...
            st.session_state[clave_cache] = partidos_ronda
        else:
            partidos_ronda = st.session_state[clave_cache]
