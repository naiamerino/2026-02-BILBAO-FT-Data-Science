...
9. Encontrar la identificación del restaurante, el nombre, el distrito y la cocina para aquellos restaurantes que pertenecen al distrito de Staten Island o Queens o Bronxor Brooklyn
   ...
   *AQUÍ METO YA UN WHERE (CONDICION) Y SELECT (LE ESPECIFICO LO QUE QUIERO). ANTES NO ESTABA ESPECIFICANDO POR LO QUE ME DEVOLVÍA TODO EL REGISTRO.*
   db["2026-02-test"].find(
...
10. Encontrar el Id. del restaurante, el nombre, el distrito y la cocina de aquellos restaurantes que lograron una puntuación que no supere los 10
    ...
    NOT MAYOR ES PRIMERO EL NOT Y ENTRE LLAVES LA CONDICIÓN QUE NIEGA
    db["2026-02-test"].find(
    {"grades.score" :
    { $not:
    {$gt : 10}
...
13. 
14. Crea un par de restaurantes que te gusten. Tendrás que buscar en google Maps los datos de las coordenadas
    ...
    insertOne VOY A METER UN ÚNICO RESTAURANTE
    no tiene todos los campos que tenían los anteriores pero da igual. 
...
14. Actualiza los restaurantes. Cambia el tipo de cocina "Ice Cream, Gelato, Yogurt, Ices" por "sweets"
    ...
    **updateMany** - QUIERO HACER MODIFICACIONES DE MUCHOS TIPOS
    El resultado que me da la consulta es el siguiente

    acknowledged: true, - lo ha hecho
    insertedId: null, - no ha creado ningun id de mongo nuevo
    matchedCount: 25, - cuantos cumplían la condición
    modifiedCount: 25, - cuantos he modificado
    upsertedCount: 0 - cuantas claves nuevas he introducido
15. Actualiza el nombre del restaurante "Wild Asia" por "Wild Wild West"
    ...
    **updateOne -** MODIFICACIÓN EN UNA CATEGORÍA
...
con db.employees.insertMany si la colección no existe la crea
...
1. Devuelve todas las empleadas de la empresa usando $match - MATCH VA SIEMPRE CON EL AGGREGATE
   db.employees.aggregate([ {$match:{ gender: 'female'}} ])
   Si hago esta misma consulta sobre la otra tabla por ejemplo (que no tiene gender) no me devolverá nada pero tampoco me dará error
...
}]) GROUP es el equivalente a GROUPBY en este caso del departamento y sobre ellos aplica la suma
3. Modifica el ejercicio anterior para que sólo devuelva datos de los empleados
Añade un where previamente 
...
db.employees.aggregate([
    { $match:{ gender:'female'}},
    { $sort:{ salary:1}} -- SORT que me ordene por salario. de forma ascenente (1) o descendente (-1)
])
