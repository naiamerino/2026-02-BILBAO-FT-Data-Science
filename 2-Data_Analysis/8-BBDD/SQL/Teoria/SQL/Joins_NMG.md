...
Quiero los pedidos con el nombre de la empresa que me lo ha hecho.

Que me las junte (INNER) por qué valor (ON cuando coincidan)

Qué quiero de ahí? SELECT de cada una de las dos lo que quiero
...
Syntax:
 ...
 Me da todos los clientes y todos los pedidos de las dos tablas
...
FROM Customers
	INNER JOIN Orders ON Customers.CustomerID = Orders.CustomerID
	INNER JOIN [Order Details] ON Orders.OrderID = [Order Details].OrderID -- Order Details está entre [] porque el nombre tiene un espacio
WHERE Customers.Country = 'UK'
